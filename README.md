...
│   ├── integrate-boot-oss       # Object storage: OssStorage facade + filesystem backend (metadata SPI)
...
Object storage ("OSS") behind one storage-agnostic facade: save content, stream it back
(including partial reads for resumable downloads), copy and delete — the classic
S3-style surface. The framework owns the content bytes and hands every
`OssObject` metadata record to a business-provided `OssMetadataStore`, so *where file
metadata lives is a business decision* (a table in the primary database, a dedicated
store, ...), while the storage backend itself stays swappable.

### What you get out of the box

- `OssStorage` — the facade: `save` / `openStream` (+ position for HTTP `Range`) /
  `readAll` / `writeTo` / `find` / `getObject` / `exists` / `copy` / `delete`.
- `FilesystemOssStorage` — the default backend: content under one root
  (`oss/` relative to the working directory by default), sharded into
  `root/<first two id chars>/<id>` directories so no directory grows unbounded. Saves
  stream through a temp file and an atomic move; length and SHA-256 checksum are
  computed on the fly; a failed metadata write rolls the content back.
- `OssObjectNotFoundException` → HTTP 404 and `OssException` → HTTP 500, both rendered
  in the shared `ResultInfo` envelope by the global exception handler — download
  endpoints need no error mapping of their own.
- Bean-driven activation: the default storage is auto-configured as soon as a
  `OssMetadataStore` bean is registered; defining any `OssStorage` bean (e.g. an
  S3/MinIO backend) makes the default back off.
...
public class InvoiceService {
    ...
    public OssObject store(MultipartFile upload) throws IOException {
        return ossStorage.save(
                OssSaveRequest.builder()
...
// Download without buffering the file in memory:
OssObject file = ossStorage.getObject(id);                          // 404 via global handler if absent
ossStorage.writeTo(id, response.getOutputStream());

// Or read it yourself (position = resume point of an HTTP Range request):
try (InputStream in = ossStorage.openStream(id, rangeFrom)) { /* ... */ }
```
...
integrate-boot:
  oss:
    filesystem:
      root: oss        # content root of the default backend; point at a durable volume in production
```

### Implementing the metadata store (your part)

Register exactly one `OssMetadataStore` bean — that is the whole opt-in, and the sample
app ships a JDBC implementation you can copy (`FileMetadataStore` in
`test/integrate-boot-test`): three methods over whatever persistence you already use.
...
public class FileMetadataStore implements OssMetadataStore {
    ...
    public void save(OssObject object) {
        jdbc.update("insert into oss_object (id, name, content_type, content_length, checksum, "
                + "directory, metadata, storage, created_at) values (?,?,?,?,?,?,?,?,?)",
    ...
    @Override
    public Optional<OssObject> find(String objectId) { /* select by id */ }

    @Override
    public void delete(String objectId) { /* delete by id; unknown ids are a no-op */ }
}
```

The contract is small and spelled out on the interface: `save` inserts a freshly
generated record (ids are UUID-based, generated by the storage layer), `find` is the
lookup behind every read, `delete` treats unknown ids as a no-op — and every
`OssObject` component (the user-metadata map included) must survive a save/find round
trip. End-to-end wiring of facade + business store is covered by `OssIT` in the sample
app.
